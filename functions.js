...
router.post('/discussion', async (req, res) => {
    ...
    let systemMsg = `Pretend you're a program that generates discussions for a chatbot. You must generate a full discussion for a chatbot, including the things the bot says and the possible answers the user can give. The bot you generate will be used for ${instructions.useCase}. The conversation of the chatbot should be in a ${instructions.tone} tone. Their company is in the ${instructions.industry} industry. There may be additional discussion points at end of the prompt. Your output should have a similar structure to: 1. Question a. Answer b. Answer c. Answer 2. Question a. answer b. answer 3. Question a. Answer. Only generate the bot conversation, and nothing else.`
...
router.post('/bot', async (req, res) => {
    ...
    let systemMsg = `Turn this conversation into a JSON: ${discussionData.message.join(' ')}. You must mirror the structure of this JSON, with the exact keys and nesting. : ${JSON.stringify(example)}`
    console.log(discussionData)
    console.log(systemMsg)
    const completion = await openai.createChatCompletion({
        model: "gpt-3.5-turbo",
