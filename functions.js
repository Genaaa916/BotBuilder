...
router.post('/discussion', async (req, res) => {
    ...
    let systemMsg = `Pretend you're a program that generates discussions for chatbot. You must generate a full discussion for a chatbot, including the things the bot says and the possible answers the user can give. The user answer options must be INSIDE each number point in the list. The user wants to implement a ${instructions.useCase} chatbot on their website. The conversation of the chatbot should be in a ${instructions.tone} tone. Their company is in the ${instructions.industry} industry. The user may have some discussion points in mind, which will be listed at the end of the prompt. Your output should be a discussion, separated by points 1. 2. etc and the possible answers nested in the points.`
...
router.post('/bot', async (req, res) => {
    ...
    if (!Array.isArray(discussionData.message)){
        return
    } else{
            const prompt = req.body.message  
    }
   let systemMsg = `Generate a JSON for a chatbot based on this conversation: ${discussionData.message.join(' ')}. The JSON should mirror this structure: ${JSON.stringify(example)}`
    console.log(discussionData)
console.log(systemMsg)
        const completion = await openai.createChatCompletion({
        model : "gpt-3.5-turbo",
